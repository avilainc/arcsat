...
impl JobQueue {
    ...
    pub async fn enqueue(&self, job: ScrapingJob) -> Result<String> {
        ...
        // Salvar job no Redis
        conn.hset::<_, _, _, ()>(&format!("job:{}", job_id), "data", &job_json).await
            .map_err(|e| arcsat_core::ArcsatError::Internal(e.to_string()))?;
    ...
    pub async fn update_status(&self, job_id: &str, status: JobStatus) -> Result<()> {
        ...
        conn.hset::<_, _, _, ()>(&format!("job:{}", job_id), "status", format!("{:?}", status)).await
            .map_err(|e| arcsat_core::ArcsatError::Internal(e.to_string()))?;
    ...
    pub async fn save_results(&self, job_id: &str, products: &[crate::models::ScrapedProduct]) -> Result<()> {
        ...
        conn.hset::<_, _, _, ()>(&format!("job:{}", job_id), "results", results_json).await
            .map_err(|e| arcsat_core::ArcsatError::Internal(e.to_string()))?;

        conn.hset::<_, _, _, ()>(&format!("job:{}", job_id), "results_count", products.len()).await
            .map_err(|e| arcsat_core::ArcsatError::Internal(e.to_string()))?;
